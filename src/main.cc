...
extern uint64_t sum_mshr_stall;
extern uint64_t count_mshr_stall_start;
extern uint64_t count_mshr_stall_end;
#ifdef PROFILING
...
//   cout << endl;
  ...
  int sum_stall = 0;
  for (uint32_t i = 0; i < NUM_TYPES; i++) {
    sum_stall += cache->STALL[i];
  }
  cout << cache->NAME << " TOTAL     STALL: " << setw(10) << sum_stall << endl;
  cout << cache->NAME << " LOAD      STALL: " << setw(10) << cache->STALL[0] << endl;
  cout << cache->NAME << " RFO       STALL: " << setw(10) << cache->STALL[1] << endl;
  cout << cache->NAME << " PREFETCH  STALL: " << setw(10) << cache->STALL[2] << endl;
  cout << cache->NAME << " WRITEBACK STALL: " << setw(10) << cache->STALL[3] << endl;

  if(cache->cache_type == IS_L1D) {
  	cout << cache->NAME << " TOTAL     STALL TIME: " << sum_mshr_stall << endl;
	cout << cache->NAME << " TOTAL     STALL START: " << count_mshr_stall_start << endl;
	cout << cache->NAME << " TOTAL     STALL END: " << count_mshr_stall_end << endl;
  }
#endif
...
void reset_cache_stats(uint32_t cpu, CACHE *cache) {
  for (uint32_t i = 0; i < NUM_TYPES; i++) {
    ...
    cache->STALL[i] = 0;
	sum_mshr_stall = 0;
	count_mshr_stall_start = 0;
	count_mshr_stall_end = 0;
